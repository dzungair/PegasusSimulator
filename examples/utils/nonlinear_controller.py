...
class NonlinearController(Backend):
    ...
    def __init__(
        ...
        self.v = np.zeros(
            (3,)
        )  # The linear velocity of the vehicle in the inertial frame
        self.a = np.zeros(
            (3,)
        )  # The linear acceleration of the vehicle in the inertial frame
    ...
    def zero_input_reference(self):
        ...
        """
        num_rotors = 4
        self.input_ref = [0.0 for i in range(num_rotors)]
    ...
    def update(self, dt: float):
        ...
        if (
            ...
            and self.total_time >= self.trajectory[self.index + 1, 0]
        ):
            self.index += 1
    ...
    def dd_pd(self, t, s, reverse=False):
        ...
        ) / np.power(s, 5) - np.exp(-np.power(t, 2) / (2 * np.power(s, 2))) / np.power(
            s, 3
        )
        z = (
            np.power(t, 2) * np.exp(-np.power(t, 2) / (2 * np.power(s, 2)))
    ...
    def ddd_pd(self, t, s, reverse=False):
        ...
        z = (3 * t * np.exp(-np.power(t, 2) / (2 * np.power(s, 2)))) / np.power(
            s, 5
        ) - (
            np.power(t, 3) * np.exp(-np.power(t, 2) / (2 * np.power(s, 2)))
        ) / np.power(
