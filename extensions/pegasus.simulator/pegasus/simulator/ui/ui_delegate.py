...
try:
    ...
    carb.log_warn(
        "ROS2 backend not available. Please install the ROS2 extension to use this feature."
    )
...
class UIDelegate:
    ...
    def set_ardupilot_directory_field(
        self, directory_field_model: ui.AbstractValueModel
    ):
    ...
    def on_load_scene(self):
        ...
        if self._scene_dropdown is not None:
            ...
            self._pegasus_sim.set_world_settings(
                **WORLD_SETTINGS[self._streaming_backend]
            )
            asyncio.ensure_future(
                self._pegasus_sim.load_environment_async(
                    SIMULATION_ENVIRONMENTS[selected_world], force_clear=True
                )
            )
            ...
            if self._vehicle_dropdown is not None and self._window is not None:
                ...
                if self._streaming_backend == BACKENDS["px4"]:
                    ...
                    px4_path = os.path.expanduser(
                        self._px4_directory_field.get_value_as_string()
                    )
                    ...
                    ardupilot_autostart = (
                        self._ardupilot_autostart_checkbox.get_value_as_bool()
                    )

                    # Read the ArduPilot path from the field
                    ardupilot_path = os.path.expanduser(
                        self._ardupilot_directory_field.get_value_as_string()
                    )

                    # Read the ArduPilot airframe from the field
                    ardupilot_airframe = (
                        self._ardupilot_airframe_field.get_value_as_string()
                    )
    ...
    def on_set_viewport_camera(self):
        ...
        if self._window:
            ...
            if camera_position is not None and camera_target is not None:
                ...
                self._pegasus_sim.set_viewport_camera(
                    eye=camera_position, target=camera_target
                )
