...
class ROS2CameraGraph(Graph):
    ...
    def __init__(self, camera_prim_path: str, config: dict = {}):
        ...
        Examples:
            ...
            >>> {"graph_evaluator": "execution",            # type of the omnigraph to create (execution-realtime, push-event)
    ...
    def initialize(self, vehicle: Vehicle):
        ...
        graph_config = {
            ...
            keys.CONNECT: [
                ...
                (
                    "create_viewport.outputs:viewport",
                    "get_render_product.inputs:viewport",
                ),
                (
                    "create_viewport.outputs:execOut",
                    "set_viewport_resolution.inputs:execIn",
                ),
                (
                    "create_viewport.outputs:viewport",
                    "set_viewport_resolution.inputs:viewport",
                ),
                ("set_viewport_resolution.outputs:execOut", "set_camera.inputs:execIn"),
                (
                    "get_render_product.outputs:renderProductPath",
                    "set_camera.inputs:renderProductPath",
                ),
            ],
            keys.SET_VALUES: [
        ...
        for camera_type in self._types:
            ...
            graph_config[keys.CONNECT] += [
                ...
                (
                    "get_render_product.outputs:renderProductPath",
                    f"{camera_helper_name}.inputs:renderProductPath",
                ),
            ]
            graph_config[keys.SET_VALUES] += [
                ...
                (
                    f"{camera_helper_name}.inputs:topicName",
                    f"{self._base_topic}/{camera_type}",
                ),
            ...
            if self._publish_labels and camera_type in [
                ...
                graph_config[keys.SET_VALUES] += [
                    ...
                    (
                        ...
                        f"{self._frame_id}/{camera_type}_labels",
                    ),
                ]
        ...
        if not valid_camera_type:
            carb.log_error(
                f"Cannot create ROS2 Camera graph, no valid camera type was selected"
            )
            return
    ...
    def camera_topic(self, camera_type: str) -> str:
        ...
        """
        return (
            f"{self._namespace}{self._base_topic}/{camera_type}"
            if camera_type in self._types
            else ""
        )
